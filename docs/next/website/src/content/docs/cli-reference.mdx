...
`STATUS` is one of `idle`, `working`, `blocked`, `done`, or `unknown`. `--agent` is a guard for the authoritative agent label. `--applies-to-source` is a guard for the active lifecycle authority source. Use `--display-agent` to change the visible name. `--model` reports the model name the agent is running; it has no clear flag, so use `--ttl-ms` to let it expire.

Metadata text is normalized before storage. Herdr trims surrounding whitespace, removes control characters, caps `--custom-status` at 32 characters, and caps `--title`, `--display-agent`, `--model`, and each `--state-label` value at 80 characters. Empty normalized values are ignored.
...
```bash
herdr agent list [--status <idle|working|blocked|done|unknown>[,...]]
...
`agent list --status` filters the response by `agent_status`. It takes one flag with comma-separated values, unioned; tokens are trimmed and deduplicated, and matching is exact lowercase. An empty or unrecognized value prints usage on stderr and exits 2. A filter that matches nothing prints a success envelope with an empty `agents` array.

`agent list` and `agent get` output includes `tab_label` and `workspace_label` alongside `tab_id` and `workspace_id`, matching what the tab bar and sidebar show. Both are display-only strings: `tab_label` falls back to the tab's positional number for un-renamed tabs, so it can disagree with the stable number encoded in `tab_id`, and `workspace_label` is not unique across same-named directories. Use ids, not labels, to target actions.
