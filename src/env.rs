...
mod tests {
    ...
    fn scrub_removes_known_herdr_vars() {
        ...
        assert!(envs.get("UNRELATED_VAR").and_then(|v| v.as_ref()).is_some());
    }
    ...
    fn scrub_herdr_vars_drift_test() {
        ...
        // - test-only fixtures, not set on any real subprocess (ARGV_CAPTURE is set
        //   inside #[cfg(test)] in src/platform/windows.rs to capture a shell's argv)
        let exemptions: &[&str] = &[
