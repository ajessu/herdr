...
impl AppState {
    ...
    // TODO(upstream-merge): route through runtime_tab_move adapter once fork
    // MoveTabLeft/MoveTabRight actions dispatch via execute_tui_navigate_action.
...
impl AppState {
    ...
    // TODO(upstream-merge): route through runtime_* adapters; upstream gates the
    // direct-mutation equivalents #[cfg(test)] but fork modal/sticky executors
    // still call these in production.
    pub fn navigate_pane_or_tab(&mut self, direction: NavDirection) {
        let Some(ws_idx) = self.active else {
            return;
        };
        let Some(tab) = self.workspaces.get(ws_idx).and_then(|ws| ws.active_tab()) else {
        ...
        let moved = if let Some(focused) = panes.iter().find(|p| p.is_focused) {
            if let Some(target) = find_in_direction(focused, direction, &panes) {
                self.focus_pane_in_workspace(ws_idx, target);
                true
            } else {
    ...
    // TODO(upstream-merge): route through runtime_tab_close adapter; upstream
    // gates this #[cfg(test)] but fork mouse middle-click and modal paths still
    // call it directly.
