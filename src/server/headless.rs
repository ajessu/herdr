...
"#,
        ...
        // TODO(upstream-merge): port 088922d assertions — upstream checks that an
        // invalid user `new_workspace` binding displaces the default (bindings end
        // up empty). This fork test config uses the modal keybind system and does
        // not set `new_workspace`, so the default binding applies and upstream's
        // `is_empty()` assertion does not transplant. Re-add equivalent coverage
        // against the fork's modal keybind config.
