...
fn pane_inner_for(info: &PaneInfo, area: Rect, multi_pane: bool) -> Rect {
    ...
    if multi_pane {
        // Honor the border set computed by `apply_pane_chrome` (which accounts
        // for pane gaps), not an unconditional `Borders::ALL`.
...
pub(super) fn compute_pane_infos(
    ...
    for info in &mut pane_infos {
        ...
        // single-pane cases and honors the gap-aware borders from
        // `apply_pane_chrome`; this loop just consumes its result.
        let pane_inner = pane_inner_for(info, area, multi_pane);
