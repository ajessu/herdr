...
pub(crate) use self::{
    ...
    sidebar::{
        ...
        collapsed_ws_window, compute_workspace_card_areas, expanded_sidebar_close_button_rect,
        expanded_sidebar_sections, expanded_sidebar_toggle_rect, is_attention_state,
...
fn resize_background_tab_panes_for_desktop(
    ...
    for (ws_idx, ws) in app.workspaces.iter().enumerate() {
        let (_, terminal_area, _) = desktop_tab_bar_and_terminal_area(app, ws, main_area);
        for (tab_idx, tab) in ws.tabs.iter().enumerate() {
            if app.active == Some(ws_idx) && tab_idx == ws.active_tab_index() {
                continue;
            }
...
fn desktop_tab_bar_and_terminal_area(
    ...
    let want_tab_row = !hide_single_tab_bar && main_area.height > 1;
    let hint_enabled = app.hint_bar != crate::config::HintBarStyle::Off;
    let want_hint_row = hint_enabled && main_area.height >= 3;
    match (want_tab_row, want_hint_row) {
        (true, true) => {
        ...
        (true, false) => {
            ...
            (tab, term, Rect::default())
        }
        (false, true) => {
            let [term, hint] =
                Layout::vertical([Constraint::Min(1), Constraint::Length(1)]).areas(main_area);
            (Rect::default(), term, hint)
        }
        (false, false) => (Rect::default(), main_area, Rect::default()),
    }
}
...
fn compute_view_internal(
    ...
    let (tab_bar_rect, terminal_area, hint_bar_rect) = app
        .active
        .and_then(|i| app.workspaces.get(i))
        .map(|ws| desktop_tab_bar_and_terminal_area(app, ws, main_area))
        .unwrap_or((Rect::default(), main_area, Rect::default()));
...
pub fn render_with_runtime_registry(
    ...
    match app.mode {
        ...
        Mode::Terminal | Mode::Pane | Mode::Tab | Mode::Move | Mode::Session | Mode::Locked => {}
    }
}
...
mod tests {
    ...
    fn keybind_help_shows_unset_for_optional_actions() {
        ...
        assert!(panes
            .iter()
            .any(|(key, label)| key == "prefix+h / alt+h" && label.as_ref() == "focus pane left"));
        assert!(panes
            .iter()
            .any(|(key, label)| key == "prefix+j / alt+j" && label.as_ref() == "focus pane down"));
        assert!(panes
            .iter()
            .any(|(key, label)| key == "prefix+k / alt+k" && label.as_ref() == "focus pane up"));
        assert!(panes
            .iter()
            .any(|(key, label)| key == "prefix+l / alt+l" && label.as_ref() == "focus pane right"));
    }
